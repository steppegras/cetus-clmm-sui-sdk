...
export class RewarderModule {
    ...
    async emissionsEveryDay(poolObjectId) {
        const currentPool = await this.sdk.Resources.getPool(poolObjectId);
    ...
    async updatePoolRewarder(poolObjectId, currentTime) {
    ...
    async posRewardersAmount(poolObjectId, positionHandle, positionId) {
        const currentTime = Date.parse(new Date().toString());
        const pool = await this.updatePoolRewarder(poolObjectId, new BN(currentTime));
    ...
    async poolRewardersAmount(account, poolObjectId) {
        const currentTime = Date.parse(new Date().toString());
        const pool = await this.updatePoolRewarder(poolObjectId, new BN(currentTime));
