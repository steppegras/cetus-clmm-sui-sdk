...
import { getMoveObjectType, getObjectFields, getObjectOwner, getObjectPreviousTransactionDigest, getObjectType, normalizeSuiAddress, normalizeSuiObjectId, TransactionBlock, } from '@mysten/sui.js';
...
import { CONST_DENOMINATOR, LaunchpadRouterModule, } from '../types/luanchpa_type';
import { PoolLiquidityCoinType, CLOCK_ADDRESS } from '../types/sui';
...
export class LaunchpadModule {
    ...
    async getInitFactoryEvent(forceRefresh = false) {
    ...
    //   return lockEvent
    // }
    async getLockNFT(nft_id, forceRefresh = false) {
    ...
    async getLockNFTList(poolType, recipient) {
        const { sdkOptions } = this._sdk;
        const result = [];
        const poolTypeWarp = extractStructTagFromType(poolType);
        try {
            const objects = (await loopToGetAllQueryEvents(this._sdk, { query: { MoveEventType: `${sdkOptions.launchpad.ido_display}::lock::LockNFTEvent` } }))?.data;
    ...
    async creatPoolTransactionPayload(params) {
        ...
        const min_sale_amount = d(params.sale_total)
            ...
            .toNumber();
        const primaryCoinInputs = (await TransactionUtil.syncBuildCoinInputForAmount(this._sdk, tx, BigInt(min_sale_amount), params.coin_type_sale));
    ...
    async creatPurchasePayload(params) {
        ...
        const tx = new TransactionBlock();
        tx.setGasBudget(this._sdk.gasConfig.GasBudgetHigh2);
        const primaryCoinInputs = (await TransactionUtil.syncBuildCoinInputForAmount(this._sdk, tx, BigInt(params.purchase_amount), params.coin_type_raise));
    ...
    async creatClaimPayload(params) {
        const { launchpad } = this.sdk.sdkOptions;
        if (this._sdk.senderAddress.length === 0) {
            throw Error('this config sdk senderAddress is empty');
        }
    ...
    async creatSettlePayload(params) {
        const { launchpad, clmm } = this.sdk.sdkOptions;
        if (this._sdk.senderAddress.length === 0) {
            throw Error('this config sdk senderAddress is empty');
        }
        ...
        if (clmm_args) {
            const initialize_sqrt_price = clmm_args.opposite
                ? TickMath.priceToSqrtPriceX64(d(1).div(clmm_args.current_price), // 0.01
                clmm_args.raise_decimals, clmm_args.sale_decimals).toString()
                : TickMath.priceToSqrtPriceX64(d(clmm_args.current_price), // 0.01
            ...
            const primaryCoinInputsR = TransactionUtil.buildCoinInputForAmount(tx, coinAssets, needAmount, needCoinType)?.transactionArgument;
    ...
    async isAdminCap(walletAddress) {
    ...
    async isWhiteListUser(whitetHandle, walletAddress) {
        const name = {
            type: 'address',
            value: walletAddress,
        };
    ...
    async getPurchaseAmount(purchaseHandle, walletAddress) {
        const name = {
            type: 'address',
            value: walletAddress,
        };
    ...
    async getPurchaseMarks(accountAddress, poolAddressArray = [], forceRefresh = true) {
        const { launchpad } = this._sdk.sdkOptions;
    ...
    async getSettleEvent(poolAddress) {
        const { launchpad } = this._sdk.sdkOptions;
