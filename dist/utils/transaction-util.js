...
import { getTransactionEffects, TransactionBlock, } from '@mysten/sui.js';
...
import { adjustForSlippage, asUintN, ClmmPoolUtil, d, Percentage, SwapUtils } from '../index';
...
export async function sendTransaction(signer, tx, onlyCalculateGas = false) {
    try {
...
export async function printTransaction(tx) {
...
export class TransactionUtil {
     ...
     * @returns
     */
    static async buildAddLiquidityTransactionForGas(sdk, allCoins, params, gasEstimateArg) {
        ...
        if (fixCoinInput !== undefined && newTx !== undefined) {
            ...
            if (isAdjustCoinA) {
                ...
                primaryCoinBInputs = TransactionUtil.buildCoinInputForAmount(newTx, allCoins, BigInt(params.amount_b), params.coinTypeB)?.transactionArgument;
            }
            else {
                params.amount_b = Number(fixAmount);
                primaryCoinAInputs = TransactionUtil.buildCoinInputForAmount(newTx, allCoins, BigInt(params.amount_a), params.coinTypeA)?.transactionArgument;
     ...
     * @returns
     */
    static async buildAddLiquidityTransaction(sdk, allCoinAsset, params) {
        if (sdk.senderAddress.length === 0) {
            throw Error('this config sdk senderAddress is empty');
        }
        const isFixToken = !('delta_liquidity' in params);
        let tx = new TransactionBlock();
        const primaryCoinAInputs = TransactionUtil.buildCoinInputForAmount(tx, allCoinAsset, BigInt(isFixToken ? params.amount_a : params.max_amount_a), params.coinTypeA)?.transactionArgument;
        const primaryCoinBInputs = TransactionUtil.buildCoinInputForAmount(tx, allCoinAsset, BigInt(isFixToken ? params.amount_b : params.max_amount_b), params.coinTypeB)?.transactionArgument;
        if (isFixToken) {
    ...
    static buildSwapTransaction(sdk, params, allCoinAsset) {
        let tx = new TransactionBlock();
        tx.setGasBudget(sdk.gasConfig.GasBudgetHigh2);
        const primaryCoinInputs = TransactionUtil.buildCoinInputForAmount(tx, allCoinAsset, BigInt(params.by_amount_in ? params.amount : params.amount_limit), params.a2b ? params.coinTypeA : params.coinTypeB)?.transactionArgument;
    ...
    static async syncBuildCoinInputForAmount(sdk, tx, amount, coinType) {
        if (sdk.senderAddress.length === 0) {
            throw Error('this config sdk senderAddress is empty');
        }
    ...
    static async calculationTxGas(sdk, tx) {
        ...
        for (let i = 0; i < params.paths.length; i += 1) {
            if (params.paths[i].poolAddress.length === 1) {
                const swapParams = {
                    ...
                    amount_limit: adjustForSlippage(new BN(params.paths[i].rawAmountLimit[0]), Percentage.fromDecimal(d(params.priceSplitPoint)), !byAmountIn).toString(),
                    swap_partner: '',
                ...
                const swapParams = {
                    ...
                    amount_0,
                    amount_1,
                    amount_limit_0: adjustForSlippage(new BN(params.paths[i].rawAmountLimit[0]), Percentage.fromDecimal(d(params.priceSplitPoint)), !byAmountIn).toString(),
                    amount_limit_1: adjustForSlippage(new BN(params.paths[i].rawAmountLimit[1]), Percentage.fromDecimal(d(params.priceSplitPoint)), !byAmountIn).toString(),
                    swap_partner: '',
                    coinTypeA: params.paths[i].coinType[0],
                    coinTypeB: params.paths[i].coinType[1],
