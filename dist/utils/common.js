...
import { Ed25519Keypair, getMoveObjectType, getObjectDeletedResponse, getObjectDisplay, getObjectFields, getObjectId, getObjectNotExistsResponse, getObjectOwner, Secp256k1Keypair, } from '@mysten/sui.js';
import BN from 'bn.js';
import { fromB64, fromHEX } from '@mysten/bcs';
import { MathUtil } from '../math';
import { PositionStatus } from '../modules/resourcesModule';
...
export function buildPosition(objects) {
    let nft = {
        creator: '',
        description: '',
        image_url: '',
...
export async function loopToGetAllQueryEvents(sdk, params) {
    let result = [];
    let cursor = null;
    while (true) {
        // eslint-disable-next-line no-await-in-loop
        const res = await sdk.fullClient.queryEvents({
            ...params,
            cursor,
        });
        if (res.data) {
...
export async function getOwnedObjects(sdk, owner, params) {
    let result = [];
    let cursor = null;
    while (true) {
        // eslint-disable-next-line no-await-in-loop
        const res = await sdk.fullClient.getOwnedObjects({
            owner,
            ...params,
            cursor,
        });
...
export async function multiGetObjects(sdk, ids, options, limit = 50) {
