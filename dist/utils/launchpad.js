...
import { getMoveObjectType, getObjectFields, getObjectId, normalizeSuiObjectId, } from '@mysten/sui.js';
import BN, { min } from 'bn.js';
import { CONST_DENOMINATOR, LaunchpadPoolActivityState } from '../types/luanchpa_type';
...
export class LauncpadUtil {
    ...
    static raiseTotalAmount(pool, saleDecimals, raiseDecimals) {
        ...
        if (saleDecimals > raiseDecimals) {
            return Number(d(pool.sale_total)
     ...
     * @param pool
     */
    static async calculatePoolPrice(sdk, pool) {
     ...
     * @returns
     */
    static async getWithdrawRaise(pool) {
     ...
     * @returns
     */
    static async getWithdrawSale(pool) {
        if (pool.pool_status === LaunchpadPoolActivityState.Ended) {
     ...
     * @returns
     */
    static async getHistoryWithdrawRaise(sdk, pool) {
        if (pool.pool_status === LaunchpadPoolActivityState.Ended) {
    ...
    static async getHistoryWithdrawSale(sdk, pool) {
        if (pool.pool_status === LaunchpadPoolActivityState.Ended) {
            const settleEvent = await sdk.Launchpad.getSettleEvent(pool.pool_address);
            if (settleEvent) {
     ...
     * @returns
     */
    static async getOverrecruitReverseAmount(sdk, pool) {
     ...
     * @returns
     */
    static async getCanPurchaseAmount(sdk, pool) {
